exports.ChannelStructures = (redisC, EventBus) ->
	...
	ServerChannelModel = ChannelModel.extend
		defaults:
			name: ""
			topic: null
			private: null
			...
			if @get("private") isnt null
			  ...
			  redisC.hget "channels:" + channelName, "isPrivate", (err, reply) =>
			    ...
			    if reply is "true" # channel is private
			      DEBUG and console.log("user attempted to join private room", channelName)
			      @set "private", true
			      callback null, true
			    else # channel is public
			      DEBUG and console.log("user attempted to join public room", channelName)
			      @set "private", false
			      callback null, false
		...
		authenticationSuccess: (client) ->
			...
			@getSocketAuthObject socket, (err, authStatus) =>
			  ...
			  socket.set "authStatus", authStatus, ->
			    DEBUG and console.log("authSucc", channelName, socket.id)
