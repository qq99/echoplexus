exports.CallServer = (sio, redisC, EventBus, Channels, ChannelModel) ->
    CALLSPACE = "/call"
    ...
    DEBUG = config.DEBUG
    CallServer = require("./AbstractServer.js").AbstractServer(sio, redisC, EventBus, Channels, ChannelModel)
        ...
        events:
            "join": (namespace, socket, channel, client, data, ack) ->
                ...
                if _.isEmpty(channel.call)
                    sio.of(CALLSPACE).in(room).emit "status:#{room}", active: true
